        ...
        for episode in tqdm(range(self.n_episodes), ncols=100):
            ...
            for hour in range(0, self.n_hours):
                ...
                for soc_bin in range(0, self.n_divisions_for_soc):
                    ...
                    Q = self.Q[soc_bin][hour][int(float(percent_honest)/0.25 - 2)]
                ...
                for soc_bin in range(0, self.n_divisions_for_soc):
                    ...
                    if hour < self.n_hours - 1:
                        delta = (reward 
                                 + self.gamma * np.max(self.Q[soc_bin][next_hour][int(float(next_percent_honest)/0.25-2)])
                                 - self.Q[soc_bin][hour][int(float(percent_honest)/0.25-2)][div_to_action_dict[soc_bin][0]])
                        self.Q[soc_bin][hour][int(float(percent_honest)/0.25-2)][div_to_action_dict[soc_bin][0]] += self.alpha * delta
                    else:
                        delta = reward - self.Q[soc_bin][hour][int(float(percent_honest)/0.25-2)][div_to_action_dict[soc_bin][0]]
                        self.Q[soc_bin][hour][int(float(percent_honest)/0.25-2)][div_to_action_dict[soc_bin][0]] += self.alpha * delta
