    ...
    def run(self):
        ...
        for episode in tqdm(range(self.n_episodes), ncols=100):
            ...
            for hour in range(0, self.n_hours):
                ...
                scaled_alpha = self.alpha * (1 - (episode/self.n_episodes)**2)
                for soc_bin in range(0, self.n_divisions_for_soc):
                    ...
                    if hour < self.n_hours - 1:
                        ...
                        self.Q[soc_bin][hour][int(float(percent_honest)/0.25-1)][div_to_action_dict[soc_bin][0]] += scaled_alpha * delta
                    else:
